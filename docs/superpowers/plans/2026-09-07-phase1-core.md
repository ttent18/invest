...
def evaluate(f: Fundamentals, c: ScreenCriteria) -> ScreenResult:
    ...
    def is_missing(value: float | None) -> bool:
        # NaN も「取得できていない」として扱う。
        # nan は比較が常に False になるため、弾かないと条件を満たしたことにされる。
...
def _equity_ratio(balance_sheet) -> float | None:
    ...
    if balance_sheet is None or getattr(balance_sheet, "empty", True):
        return None
    try:
        equity = _as_float(balance_sheet.loc["Stockholders Equity"].iloc[0])
        total = _as_float(balance_sheet.loc["Total Assets"].iloc[0])
    except (KeyError, IndexError, TypeError, ValueError):
        return None
    # _as_float は NaN を None にする。float(nan) は例外を出さず、
    # nan == 0 も False になるため、ここで弾かないと nan がそのまま返る。
...
def fetch_fundamentals(code: str) -> Fundamentals:
    ...
    try:
        ...
        # 例外の型名を残す。通信断とプログラミングエラーを後から区別するため
        raise MarketDataError(
            f"{symbol} の取得に失敗しました: {type(exc).__name__}: {exc}"
        ) from exc
...
# 日本株の「その日」は日本時間で決める。
# GitHub Actions は UTC で動くため、date.today() だと前日の日付になる
# （cron は日曜21:00 UTC = 月曜6:00 JST）。
JST = ZoneInfo("Asia/Tokyo")

# 取得成功率がこれを下回ったら書き込まない。
# select_screened は「最新の as_of」の行だけを候補にするため、
# 大量に失敗したバッチを書くと、前回の完全なデータが見えなくなり、
# 候補が激減していることに誰も気づかない。書かなければ前回のデータが残る。
...
def run(symbols: list[str], conn, as_of: date) -> tuple[int, int, bool]:
    """全銘柄を取得して保存する。戻り値は (取得件数, 失敗件数, 書き込んだか)。

    1銘柄の失敗で全体を止めない。失敗は data_gaps に記録する。
    取得成功率が SUCCESS_RATE_THRESHOLD 未満なら書き込まない。
    """
    ...
    if success_rate < SUCCESS_RATE_THRESHOLD:
        record_gap(
            ...
            detail=(
                f"取得率が低いため書き込みを見送りました: "
                f"成功 {len(fetched)}/{total} = {success_rate:.0%}"
            ),
        )
        return len(fetched), failed, False

    upsert_fundamentals(conn, fetched, as_of)
    return len(fetched), failed, True
...
def main() -> int:
    ...
    if written:
        print("保存しました")
        return 0
    print("取得率が低いため保存を見送りました（前回のデータを残します）")
    return 1
    ...
    steps:
      ...
      - run: uv run python -m investment.jobs.run_screen
        env:
          ...
          # このプロジェクトはパッケージとしてインストールされていないため、
          # `python -m investment...` が import できるよう src を明示的に追加する。
          # pytest は pyproject.toml の pythonpath=["src"] で解決されるが、
          # その設定は python -m には効かない。
          PYTHONPATH: src
```
